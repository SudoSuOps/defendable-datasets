# Legal Letter Templates v1

Legal and professional correspondence corpus for notices, demand letters, dispute responses, and structured template generation.

- Dataset ID: legal_letter_templates_v1
- Version: 1.0.0
- Status: verified
- Access: public
- License: mit
- Records: 9160
- Size bytes: 109794304
- Source type: nas
- Validation: passed

## Provenance

Indexed from Synology NAS legal consumer corpus with SHA256 and record-count receipt. Curated on sovereign bare-metal RTX 6000 fleet and RTX 3090 systems.

## Compute Provenance

Curated on sovereign bare-metal RTX 6000 fleet and RTX 3090 systems.

## Intended Use

Generate editable professional letter drafts with clear disclaimers.

## Not Intended Use

Not legal advice or jurisdiction-specific counsel.

## Files

- legal_consumer_all_jsonl: defendable-storage://legal_letter_templates_v1/legal_consumer_all_jsonl (9160 records, sha256 3d3246c9e288340acaf2c2c4e9e1d7747bcf2db931b8a4709f1ebe01fc5b9702)

Doctrine: No proof, no honey.
